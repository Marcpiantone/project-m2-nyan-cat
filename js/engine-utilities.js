...
const nextEnemySpot = (enemies) => {
  // enemySpots will refer to the number of spots available (can you calculate it? from the space available in the screen = 375px(gamewidth)/75px(enemy width))
  ...
  while (candidate === undefined || spotsTaken[candidate]) {
    // candidate is assigned a random number between 0 and enemySpots (not including enemySpots). (what number is enemySpots? between 0 and <=5)
